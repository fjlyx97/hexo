...
# 什么是安全
通信过程具备了四个特性，就可以认为是**安全**，分别是：机密性、完整性、身份认证、不可否认。只有具备了四个特性，通信双方的利益才有保障，才能算得上是真正的安全。
- 机密性：对数据保密，不能让不相干的人看到
- 完整性：数据在传输过程没有被篡改
- 身份认证：确认对方真实身份，保证消息只发给可信的人
- 不可否认：不能否认发生过的行为

# HTTPS
Https的秘密在于这个s，它把Http下层的传输协议由TCP/IP换成了SSL/TLS，让Http运行在了安全的SSL/TLS协议上，收发报文不再使用Socket API，而是调用专门的安全接口。

## TLS
TLS密码套件命令很规范，格式很固定。基本就是:密钥交互算法+签名算法+对称加密算法+摘要算法
- ECDHE-RSA-AES256-GCM-SHA384
- 对应：握手时使用 ECDHE 算法进行密钥交换，用 RSA 签名和身份认证，握手后的通信使用 AES 对称算法，密钥长度 256 位，分组模式是 GCM，摘要算法 SHA384 用于消息认证和产生随机数

# HTTP2.0
## 头部压缩
一般报文的Header都会携带UserAgent，Cookie，Accept，Server等许多固定的头字段。但是Http1.0中的头字段**content-Encoding**只有指定Body的编码方式，如用gzip压缩来节约带宽，而对非Body部门没有处理。

Http2.0对头部进行了压缩，使用的是专门开发的HPACK算法，可以达到50-90的高压缩率。

## 二进制格式
Http2.0去除了1.0中的文本传输，而改成了二进制传输。原先的Header被封装成了Frame，用Header帧存放头数据。Data帧存放实体数据。

## 虚拟的流
Http2.0中，同一个往返的帧会分配一个唯一的流ID，可以把它想象成一个虚拟的**数据流**，HTTP2.0可以在同一个TCP连接中，用流同时发送多个碎片化消息。这就是常说的**多路复用**，多个往返通信复用同一个连接来处理。

Http2.0服务器还可以主动推送消息，比如在浏览器请求Html的时候，就提前把一些JS、CSS文件发给客户端，减少等待的延时。